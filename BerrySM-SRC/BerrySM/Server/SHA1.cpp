...
void CXYZ::Transform(UINT_32* pState, const UINT_8* pBuffer)
{
...
void CXYZ::Update(const UINT_8* pbData, UINT_32 uLen)
{
...
bool CXYZ::HashFile(const TCHAR* tszFileName)
{
...
bool CXYZ::ReportHash(char* tszReport, REPORT_TYPE rtReportType) const
{
...
bool CXYZ::ReportHashStl(char* strOut, REPORT_TYPE rtReportType) const
{
	char tszOut[84];
	const bool bResult = ReportHash(tszOut, rtReportType);
	if(bResult) strcpy(strOut, tszOut);
	return bResult;
}
#endif

// Get the raw message digest
bool CXYZ::GetHash(UINT_8* pbDest) const
{
